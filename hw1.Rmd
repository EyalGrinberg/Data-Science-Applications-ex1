...
```{r}
passwords %>% filter(category == "fluffy") %>% select(password)
```

Filter only passwords in the "sport" and "cool-macho" and select only the `rank`, `password`, `category` and `strength` columns:

```{r}
passwords %>% filter(category %in% c("sport", "cool-macho")) %>% select(rank, password, category, strength)
```

Same as above, but sort by `strength`, from weakest to strongest (and do try to make this in a single pipe):

```{r}
passwords %>% filter(category %in% c("sport", "cool-macho")) %>% select(rank, password, category, strength) %>% arrange(strength)
```
...
```{r}
passwords <- passwords %>% mutate(is_123 = str_detect(password, "123"))
```
...
```{r}
passwords_with_123 <- passwords %>% filter(is_123 == T) %>% pull(password) 
...
```{r}
passwords %>% group_by(category) %>%
   summarise(mean_strength = mean(strength, na.rm = T)) %>% arrange(-mean_strength)
```
...
passwords %>%
  group_by(category) %>%
  summarise(mean_strength = mean(strength, na.rm = T), n = n()) 
```
...
passwords <- passwords %>%
  mutate(time_unit_seconds = case_when(
     ...
     time_unit == 'months' ~ 2629746,
     time_unit == 'years' ~ 31536000,
     TRUE ~ NA_real_
   ))
```
...
```{r}
passwords <- passwords %>%
   mutate(time_unit_seconds = case_match(
     time_unit,
     'seconds' ~ 1,
     'minutes' ~ 60,
     'hours' ~ 3600,
...
category_mean_strength <- function(categ) {
   passwords %>%
      ...
      summarise(mean(rank), mean(strength), mean(time_to_crack_sec_log)) %>%
      mutate(categ = categ) %>%
      as_tibble()
}
...
```{r}
summary(lm_obj)$coefficients['x2','Pr(>|t|)']
```
